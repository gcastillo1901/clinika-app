...
export default function CrearPacienteScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    const camposObligatorios: (keyof FormPaciente)[] = ['nombre', 'apellido', 'cedula', 'fecha_nacimiento', 'sexo', 'telefono'];
    const faltan = camposObligatorios.filter((campo) => !form[campo]);

    if (faltan.length > 0) {
      Alert.alert('Faltan campos obligatorios', `Complete: ${faltan.join(', ')}`);
      return;
    }
          ...
          {Platform.OS === 'web' ? (
            ...
            <>
              <Pressable onPress={() => setShowDatePicker(true)} style={styles.input}>
                <Text style={{ color: form.fecha_nacimiento ? '#000' : '#aaa' }}>
...
<View style={styles.inputGroup}>
  ...
  <TextInput
    ...
    onChangeText={(text) => handleChange('nombre_contacto_emergencia', text)}
  />
</View>
...
<View style={styles.inputGroup}>
  ...
  <TextInput
    ...
    onChangeText={(text) => handleChange('telefono_contacto_emergencia', text)}
  />
</View>
